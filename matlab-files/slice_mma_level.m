function [thetanew, psinew, Hnew, accep, ratenew, Ncrumbs] = ...
    slice_mma_level(X, y, theta, psi, N, w, c0, rate, H, ...
...
%     G = diag(std(params));
    ...
    thetanew(1,:) = newparams(1:Ntheta);
      ...
      rescale_psi = (2-lb)/(1+exp(-psinew(1,1))) + lb;
    ...
    xiparams = zeros(1, size(theta,2)+1);
    ...
    for i = 1:(N+1)
        ...
        for d = 1:(size(theta,2)+1)
            xiparams(1,d) = crumb(1,d) + c * G(d,d) * randn;
            r=min(1, normpdf(xiparams(1,d), crumb(1,d), c * G(d,d) ) /...
                normpdf( crumb(1,d), xiparams(1,d), c * G(d,d) ) );
            if rand > r
                xiparams(1,d) = crumb(1,d);
            end
        end
        
         xitheta = xiparams(1:Ntheta);
           xipsi = xiparams(Ntheta + 1);
           rescale_psi = (2-lb)/(1+exp(-xipsi)) + lb;
        ...
        while ((Hxi == Inf || Hxi == -Inf || imag(Hxi) ~= 0 || isnan(Hxi) ) || Hxi > u) && ...
            ...
            for d = 1:(1+size(theta,2))
                xiparams(1,d) = crumb(1,d) + (c/ncrumbs) * G(d,d) * randn;
                r=min(1, normpdf(xiparams(1,d), crumb(1,d), (c/ncrumbs) * G(d,d) ) /...
                    normpdf( crumb(1,d), xiparams(1,d), (c/ncrumbs) * G(d,d) ) );
                if rand > r
                    xiparams(1,d) = crumb(1,d);
                end
            end
