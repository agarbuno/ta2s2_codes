function [theta, psi, H, j, w, Theta, Accep, Tvec] = slice_parallel_aims_opt(X, y, gamma, alpha, N,...
...
while (delta == 0 || isnan(delta)) && counter < maxSamp
    ...
    indexnan = (isnan(H));
    psi = psi(~(indexinf+ indexminf+ indeximag + indexnan),:);
    theta = theta(~(indexinf+ indexminf+ indeximag +indexnan),:);
    H = H(~(indexinf+ indexminf+ indeximag+ indexnan),:);
...
if N > length(H)
    indexes = randsample(length(H), N - length(H), true);
    H = [H ; H(indexes,:)];
...
%     end    
    ...
    parfor i = 1:length(Ns)
        ...
        [ thetanew_sampled{i}, psinew_sampled{i}, Hnew_sampled{i}, ...
            ...
            slice_parallel_annealing_level(X, y, theta, psi, ...
...
%     figure(1); clf; hist(Ncrumbs_new);
%     figure(2); clf; boxplot(theta);
%     figure(3); clf; stem(Ns);
%     figure(4); clf; bar(std(theta));
%     pause
